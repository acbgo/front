...
<!--      <el-button style="position: relative;left: 1050px;top: -20px" type="danger" icon="el-icon-remove-outline" >批量删除</el-button>-->
  ...
  <el-table
    ...
    <el-table-column label="图片名称" prop="graphPath">
      <template slot-scope="scope">
        <img :src="require(`../../assets/room/${scope.row.graphPath}`)" style="height: 50px">
      </template>
    </el-table-column>
    <el-table-column
      align="right"
...
import Im from '../userPage/Chat.vue'
...
export default {
  ...
  methods: {
    receive () {
      axios.get('http://10.24.3.53:8181/findRooms?h_id=' + this.h_id).then(res => {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.operation === '修改') {
            const url = 'http://10.24.3.53:8181/updateRoom?HotelName=' + this.hotelName +
          ...
          if (this.operation === '添加') {
            const url = 'http://10.24.3.53:8181/insertRoom?HotelName=' + this.hotelName +
              '&RoomNumber=' + this.ruleForm.roomNumber + '&graphPath=' + this.ruleForm.graphPath +
    ...
    handleDelete (index, row) {
      this.operation = '删除'
      const url = 'http://10.24.3.53:8181/deleteRoom?id=' + row.id
      this.operation = '删除'
      this.send(row, url)
      this.reload()
    },
