<template>
  <div class="Header" style="background-color: #d9ecff" id="head">
    ...
    <div class="Body" style="padding: 21px; margin-bottom: 50px;">
      <el-row>
        <el-col :span="4">
          <el-card style="width: 300px; height: 300px; color: #333; margin-top: 50px; margin-left: 50px">
        ...
        <el-col :span="20">
          <div style="width: 800px; margin: 0 auto; background-color: white;
            ...
            <textarea v-model="text" style="height: 160px; width: 95%; padding: 20px; border: none; border-top: 1px solid #ccc;
             border-bottom: 1px solid #ccc; outline: none" placeholder="请输入信息"></textarea>
...
export default {
  ...
  created () {
    this.init()
    this.componentName = MainPage.document.getElementById('head')
  },
  mounted () {
    ...
    send () {
      ...
      if (!this.text) {
        ...
        if (typeof (WebSocket) === 'undefined') {
          ...
          // 组装待发送的消息 json
          // {"from": "zhang", "to": "manager", "text": "聊天文本"}
    ...
    init () {
      ...
      if (typeof (WebSocket) === 'undefined') {
        ...
        socket.onmessage = function (msg) {
          ...
          let data = JSON.parse(msg.data) // 对收到的json数据进行解析， 类似这样的： {"users": [{"username": "zhang"},{ "username": "manager"}]}
