...
export default {
  ...
  methods: {
    receive () {
      axios.get('http://10.24.168.173:8181/findallroomtype').then(res => {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.operation === '修改') {
            // 发送修改后的ruleForm
            const url = 'http://10.24.168.173:8181/updateRoomType?room_type=' + this.ruleForm.room_type +
          ...
          if (this.operation === '添加') {
            // 发送添加的ruleForm
            const url = 'http://10.24.168.173:8181/insertRoomType?room_type=' + this.ruleForm.room_type +
              '&area=' + this.ruleForm.area + '&bed_number=' + this.ruleForm.bed_number + '&price=' +
    ...
    handleDelete (index, row) {
      ...
      // 发送删除的数据
      const url = 'http://10.24.168.173:8181/deleteRoomType?id=' + row.id
      this.operation = '删除'
      this.send(row, url)
      this.reload()
